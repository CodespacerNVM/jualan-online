@props(['elementId', 'elementName', 'optionsData', 'selected' => []])

@php($selected = json_encode(is_string($selected) ? explode(',', $selected) : $selected))


<div class="w-full" x-data="alpineMuliSelect({ selected: JSON.parse('{{ $selected }}'), elementId: '{{ $elementId }}' })">
    <!-- Select Options -->
    <select class="hidden" id="{{ $elementId }}">
        @foreach ($optionsData as $data)
            <option value="{{ $data['key'] }}" data-search="{{ $data['value'] }}">{{ $data['value'] }}</option>
        @endforeach
    </select>

    <div class="w-full flex flex-col items-center mx-auto" @keyup.alt="toggle">
        <!-- Selected Teams -->
        <input name="{{ $elementName }}" type="hidden" x-bind:value="selectedValues()" class="cursor-pinter">
